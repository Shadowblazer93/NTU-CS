...
def interleaveQueue(q:Queue):
    ...
    for _ in range(size): temp.push(q.dequeue())
    for _ in range(size): q.enqueue(temp.pop())
    for _ in range(mid): q.enqueue(q.dequeue())
    for _ in range(mid): temp.push(q.dequeue())
    for _ in range(mid):
        q.enqueue(temp.pop())
        q.enqueue(q.dequeue())
